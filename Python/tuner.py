...
if __name__ == "__main__":
    ...
    # Printing the last generation to txt
    # weightedLastPop = []
    # for individual in population:
    #     Kp = individual[0]; Ki = individual[1]; Kd = individual[2]/40;
    #     sys = tf([a, b],[1, c, d]) * tf([Kd, Kp, Ki],[1, 0])
    ...
    #     fitnessVal = 1.0/(1.0+findITAE(reference,y,t))
    #     if (math.isnan(fitnessVal) == True):
    #         fitnessVal = 0
    #     pair = [individual, fitnessVal]
    #     weightedLastPop.append(pair)
    
    # file = open("Output.txt", "w")
    # file.write("Kp\tKi\tKd\n")
    # file.write(str(bestFitnessInd)+"\n")
    # for i in xrange(POPULATION_SIZE):
    #     # print weightedLastPop[i]
    #     file.write(str(weightedLastPop[i]))
    #     file.write("\n")
    # file.close()


# controller = lti([1], [1, 0])
# reference = 1
# error = []
# sys = lti([1], [1, 5]) 
# t, y = step2(sys)
